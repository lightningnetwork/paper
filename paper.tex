...
of transactions inside this network. The only transactions that are broadcast
...
Mark Friedenbach has proposed that Sequence Numbers can be enforcible via a
...
Alice (while canceling the HTLC). For Alice, she must broadcast the ``Timeout''
...
trade off exists between locking up transaction fees on each hop versus the
...
\begin{figure}[H]
	...
	\caption{Dave creates a path back to Alice after Alice fails to send
		...
		hash(R) is never broadcast by Dave, because Carol did not
...
other timelocked transactions become valid.
...
The risk model of this attack occurring is similar to that of miners colluding
...
\begin{itemize}
	...
	\item Cross-Chain Payments. So long as there are similar hash-functions
		...
		the destination chain. Similarly, the receiver does not have to
